...
ALTER TABLE users ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'viewer' NOT NULL CHECK (role IN ('admin', 'user', 'viewer'));
...
CREATE INDEX IF NOT EXISTS idx_user_permissions_vlan_id ON user_permissions(vlan_id);
CREATE INDEX IF NOT EXISTS idx_user_permissions_subnet_id ON user_permissions(subnet_id);

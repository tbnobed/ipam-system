...
export class NotificationService {
  ...
  private setupDefaultChannels() {
    ...
    this.notificationChannels.set('email', {
      type: 'email',
      enabled: false, // Disabled until SMTP is configured
      config: {
  ...
  private async sendEmail(config: any, event: AlertEvent): Promise<void> {
    ...
    console.log(`📧 [EMAIL] ${event.severity.toUpperCase()}: ${event.message}`);
    console.log(`📧 Would send to: ${config.toEmails.join(', ')}`);
